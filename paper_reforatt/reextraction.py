...
class TextDataSet:
    ...
    def _extract_inf(self,line):
        ...
        padding_num = self._num_timesteps - len(ids_last_sentence)
        ids_last_sentence = ids_last_sentence + [self._vocab.unk for i in range(padding_num)]
        return  ids_sentence,ids_next_sentence,ids_last_sentence
    ...
    def _parse_file(self, filename):
        ...
        for j,line in enumerate(self._open_csv(filename)):
            if j==line_index[index_index] and data_index==int(line[0]):
                ids_sentence, ids_next_sentence, ids_last_sentence = self._extract_inf(line)
                re_1.append(ids_sentence + ids_next_sentence + ids_last_sentence)
                data_index += 1
                index_index+=1
            elif j==line_index[index_index] and data_index!=int(line[0]):
                re_1.append([0 for _ in range(150)])
                data_index+=1
                index_index+=1
        index_index = 0
        data_index = 1
        for j,line in enumerate(self._open_csv(filename)):
            ...
            if j==line_index[index_index]+1 and data_index==int(line[0]):
                ...
                ids_sentence, ids_next_sentence, ids_last_sentence = self._extract_inf(line)
                re_2.append(ids_sentence + ids_next_sentence + ids_last_sentence)
                data_index += 1
                index_index+=1
            elif (j==line_index[index_index]+1 and data_index!=int(line[0])) or j+1>line_num:
                re_2.append([0 for _ in range(150)])
                data_index += 1
                index_index += 1
        index_index = 0
        data_index = 1
        for j,line in enumerate(self._open_csv(filename)):
            if int(data_index) == int(data_last_index) + 1:
                ...
                ids_sentence, ids_next_sentence, ids_last_sentence = self._extract_inf(line)
                re_3.append(ids_sentence + ids_next_sentence + ids_last_sentence)
                data_index += 1
                index_index+=1
            elif (j==line_index[index_index]+2 and data_index!=int(line[0])) or j+2>line_num:
                re_3.append([0 for _ in range(150)])
                data_index += 1
                index_index += 1
        index_index = 0
        data_index = 1
        for j,line in enumerate(self._open_csv(filename)):
            if int(data_index) == int(data_last_index) + 1:
                ...
                ids_sentence, ids_next_sentence, ids_last_sentence = self._extract_inf(line)
                re_4.append(ids_sentence + ids_next_sentence + ids_last_sentence)
                data_index += 1
                index_index+=1
            elif (j==line_index[index_index]+3 and data_index!=int(line[0])) or j+3>line_num:
                re_4.append([0 for _ in range(150)])
                data_index += 1
                index_index += 1
        index_index = 0
        data_index = 1
        for j,line in enumerate(self._open_csv(filename)):
            if int(data_index)==int(data_last_index)+1:
                ...
                ids_sentence, ids_next_sentence, ids_last_sentence = self._extract_inf(line)
                re_5.append(ids_sentence + ids_next_sentence + ids_last_sentence)
                data_index += 1
                index_index+=1
            elif (j==line_index[index_index]+4 and data_index!=int(line[0])) or j+4>line_num:
                re_5.append([0 for _ in range(150)])
                data_index += 1
                index_index += 1
...
# 创建网络模型
# 用于数据集编码的操作
def lstm_encoder(embed_inputs,cell,batch_size):
...
def attention_re(last1,last2,last3,last4,last5,outputs):
...
def create_model(hps,vocab_size,num_classes,paper=True):
    ...
    cell=relstm.multi_lstm(hps,keep_prob)
    ...
    # 这里的last在第二维上（可以理解为列）取最后一个数据（即在LSTM中只取最后一层的输出结果），维数为[batch_size,lstm_output[-1](32)]
    ...
    with tf.Session() as sess:
        ...
        for i in range(num_train_steps):
            ...
            if i==num_train_steps-1:
                ...
                '''
                saver.save(sess,'./output/lstm_fbatt/lstm+fbatt')
